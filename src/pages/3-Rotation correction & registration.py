...
with col1:
    ...
    if os.path.exists(segmented_image_path) and os.path.exists(mask_path):
        ...
        segmented_image = get_segmented("results/segmentation/source_image.jpg", mask_path)

        segmented_pil_image = Image.fromarray(segmented_image)
        segmented_pil_image.save("results/segmentation/segmented_image.jpg")

        aligned = alignment(segmented_image, np.array(mask))
        ...
        left_hemisphere_path = "results/segmentation/left_hemisphere.jpg"
        right_hemisphere_path = "results/segmentation/right_hemisphere.jpg"

        if os.path.exists(left_hemisphere_path):
            left_hemisphere = Image.open(left_hemisphere_path)
            rotated_left = rotate_image(left_hemisphere, rotation_degrees)
            rotated_left.save("results/alignment/aligned_left_hemisphere.jpg")

        if os.path.exists(right_hemisphere_path):
            right_hemisphere = Image.open(right_hemisphere_path)
            rotated_right = rotate_image(right_hemisphere, rotation_degrees)
            rotated_right.save("results/alignment/aligned_right_hemisphere.jpg")
